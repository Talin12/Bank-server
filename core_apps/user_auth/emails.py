...
def send_otp_email(email_address, otp): # Renamed argument to avoid conflict
    ...
    context = {
        'otp': otp,
        'expiry_time': int(settings.OTP_EXPIRATION.total_seconds() // 60), # Clean integer
        'site_name': settings.SITE_NAME,
    }
    html_email = render_to_string("emails/otp_email.html", context)
    plain_email = strip_tags(html_email)

    # Use a distinct variable name for the message object
